...
public enum Authenticator {
    ...
    */
    ...
    case HMAC(key: [UInt8], variant: Deliria.HMAC.Variant)
    ...
    */
    public func authenticate(message: [UInt8]) throws -> [UInt8] {
        ...
        case .Poly1305(let key):
            guard let auth = Deliria.Poly1305(key: key)?.authenticate(message) else {
                throw Error.AuthenticateError
            }
            return auth
        case .HMAC(let key, let variant):
            guard let auth = Deliria.HMAC(key: key, variant: variant)?.authenticate(message) else {
                throw Error.AuthenticateError
            }
            return auth
        }
